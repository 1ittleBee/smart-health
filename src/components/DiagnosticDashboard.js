    ...
    const init = async () => {
      if (window.ethereum) {
        ...
        try {
          ...
          // Ensure deployed network exists
          const deployedPatientRegistryNetwork =
            patientRegistryABI.networks[networkId];
          if (
            !deployedPatientRegistryNetwork ||
            !deployedPatientRegistryNetwork.address
          ) {
            console.error(
              "PatientRegistry contract not deployed to this network"
            );
            return;
          }
  ...
  useEffect(() => {
    const fetchPatients = async () => {
      try {
        ...
        const patientAddresses = await recordContractInstance.methods
          .getRecords()
          .call();
        ...
        const patientNames = await Promise.all(
          patientAddresses.map(async (patientAddress) => {
            const patientDetails = await patientRegistryContractInstance.methods
  ...
  return (
    <div>
      ...
      <div className="bg-gradient-to-b from-purple-500 to-purple-800 p-4 sm:p-10 font-mono text-white h-screen flex flex-col justify-center items-center">
        ...
        <p className="text-xl sm:text-2xl mb-4 text-yellow-300">

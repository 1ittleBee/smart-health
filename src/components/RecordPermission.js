...
function RecordPermission() {
  ...
  const [patientRegistryContractInstance, setPatientRegistryContractInstance] =
    useState(null);
    ...
    const init = async () => {
      if (window.ethereum) {
        ...
        try {
          ...
          const deployedPatientRegistryNetwork =
            PatientRegistryABI.networks[networkId]; // Use patient registry ABI
          ...
          const patientRegistryContract = new web3Instance.eth.Contract(
            ...
            deployedPatientRegistryNetwork &&
              deployedPatientRegistryNetwork.address
          );
          ...
          patientRegistryContractInstance.events
            .PatientRegistered({ fromBlock: "latest" })
            .on("data", async (event) => {
              // Retrieve the patient's name from the event data
              const { patientAddress: registeredAddress, name } =
                event.returnValues;

              // Update the state with the patient's name
              if (
                registeredAddress.toLowerCase() === patientAddress.toLowerCase()
              ) {
                setPatientName(name);
              }
            })
            .on("error", (error) => {
              console.error(
                "Error listening for PatientRegistered event:",
                error
              );
            });
        } catch (error) {
  ...
  return (
    <div>
      <NavBar_Logout />
      <div className="bg-gradient-to-b from-purple-500 to-purple-800 min-h-screen flex items-center justify-center p-4 font-mono text-white">
        <div className="w-full max-w-6xl bg-gray-900 p-20 rounded-lg shadow-lg">
          <h1 className="text-3xl mb-6 font-bold text-center text-yellow-300">
            Record Permission
          </h1>
          <div className="flex flex-col w-full mb-4">
            ...
            />
            {/* <button className="px-4 py-2 mt-2 bg-teal-500 hover:bg-teal-600 text-white rounded-md transition duration-200">
              Fetch Name
            </button> */}
